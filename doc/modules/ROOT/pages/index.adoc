...
This will create the Hazelcast cluster and one `LoadBalancer` service. You can also use `NodePort` service, for this change the `discoveryServiceType` to `NodePort`.
...
It is also possible to use `NodePort` type for `discoveryServiceType`. Operator will create the service of type `NodePort` for discovery.
For `memberAccess` it is also possible to use `NodePortExternalIP` that allows the client to access Hazelcast member with the `NodePort` service and the node external IP/hostname, or `NodePortNodeName` that allows the client to access Hazelcast member with the `NodePort` service and the node name.
