...
This guide will explain how to use Expose Extarnally feature of Hazelcast Platform Operator to connect to the Hazelcast cluster running on Kubernetes from Outside.

include::{templates-url}/link-to-repo.adoc[]

== What You’ll Learn

In this guide, you will learn how to enable and use Expose Extarnally feature of Hazelcast Platform Operator.
...
There are two available options for Expose Extarnally feature of Hazelcast Platform Operator:
...
The first option is to use the `Unisocket` type. This will use standard Kubernetes mechanism, that will automatically load balance the traffic to Hazelcast members.
...
Run the following command to create the Hazelcast cluster with Expose Externally feature enabled and Unisocket type.
...
This will create the Hazelcast cluster and one `LoadBalancer` services. You can also use `NodePort` service, for this change the `discoveryServiceType` to `NodePort`.
...
[source, shell]
----
$ kubectl get servive my-hazelcast
...
To delete clean up the created resources remove the Hazelcast Custom Resource.
...
Run the following command to create the Hazelcast cluster with Expose Externally feature enabled and Smart type.
...
This will create the Hazelcast cluster and one `LoadBalancer` services for discovery and separate `LoadBalancer` services for each pod.
